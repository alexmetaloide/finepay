...
function SummaryCard({ title, value, icon: Icon, color, isPercentage = false, target }: any) {
  const displayValue = isPercentage ? ((value / target) * 100).toFixed(1) + '%' : `R$ ${value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-4">
                {bills.sort((a: any, b: any) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()).map((bill: any) => {
